class Game
  ...
  def play(move_1, result)
    move_1 = to_move(move_1)
    move_2 = determine_move(move_1, result)
  ...
  MOVES = {
    # original => [loose win]
    "rock" => %w[paper scissors],
    "paper" => %w[scissors rock],
    "scissors" => %w[rock paper]
  }

  def determine_move(move1, result)
    case result
    when "X" # loose
      MOVES[move1][1]
    when "Y" # draw
